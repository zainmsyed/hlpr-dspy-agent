...
class TestCliProvidersContract:
    ...
    def test_providers_add_local(self):
        ...
        result = runner.invoke(
            app,
            ["providers", "add", "test_local", "--type", "local", "--model", "llama2:7b"],
        )
    ...
    def test_providers_add_openai(self):
        ...
        result = runner.invoke(
            app,
            ["providers", "add", "test_openai", "--type", "openai", "--model", "gpt-4", "--api-key", "test_key"],
        )
    ...
    def test_providers_add_duplicate_id(self):
        ...
        result1 = runner.invoke(
            app,
            ["providers", "add", "duplicate_test", "--type", "local", "--model", "test_model"],
        )
    ...
    def test_providers_test_connection(self):
        ...
        result1 = runner.invoke(
            app,
            ["providers", "add", "test_conn", "--type", "local", "--model", "test_model"],
        )

        if result1.exit_code == 0:
            # Then test connection
            result2 = runner.invoke(app, ["providers", "test", "test_conn"])
